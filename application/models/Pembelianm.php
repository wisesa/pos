...
class Pembelianm extends CI_Model
{
	...
	{
		$sql = "  SELECT p.id as id,p.id_supplier,p.created_on,pd.id_produk,pd.harga,pd.jumlah,u.nama as nama_supplier,pr.nama as nama_produk,batal
		from " . $this->_table . " p left outer join
		" . $this->_table_detail . "  pd on p.id=pd.id_pembelian left outer join
		user u on p.id_supplier=u.id left outer join
